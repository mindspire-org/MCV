...
export async function stockSummary(req: Request, res: Response){
  ...
  const match: any = {}
  if (locationId) match.locationId = mongoose.Types.ObjectId.isValid(locationId) ? new mongoose.Types.ObjectId(locationId) : locationId
  if (itemId) match.itemId = mongoose.Types.ObjectId.isValid(itemId) ? new mongoose.Types.ObjectId(itemId) : itemId
...
export async function receive(req: Request, res: Response){
  ...
  for (const l of data.lines){
    const lot = await HospitalStoreLot.findOneAndUpdate(
      ...
      { $setOnInsert: { itemId: l.itemId, locationId: data.locationId, vendorId: data.vendorId, receivedAt: data.date, lotNo: l.lotNo, expiryDate: l.expiryDate }, $set: { unitCost: l.unitCost }, $inc: { qtyOnHand: l.qty } },
  ...
  for (const l of data.lines){
    ...
    for (const c of consumed){
      const lot = await HospitalStoreLot.findOneAndUpdate(
        ...
        { $setOnInsert: { itemId: l.itemId, locationId: data.toLocationId, receivedAt: data.date, lotNo: c.lotNo, expiryDate: c.expiryDate }, $set: { unitCost: c.unitCost }, $inc: { qtyOnHand: c.qty } },
        { upsert: true, new: true }
      )
