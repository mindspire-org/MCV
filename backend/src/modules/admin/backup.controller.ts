...
function deserializeValue(v: any): any {
  ...
  if (isPlainObject(v) && "$oid" in v && typeof v.$oid === "string")
    return new Types.ObjectId(v.$oid);
  if (isPlainObject(v) && "$date" in v && typeof v.$date === "string")
...
async function listCollectionNames() {
  ...
  return cols.map((c) => c.name).filter((name) => !name.startsWith("system."));
}
...
export async function exportAll(req: Request, res: Response) {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="backup-${dbName}-${stamp}.json"`,
  );
...
export async function purgeAll(_req: Request, res: Response) {
  const ok =
    String((_req.body as any)?.confirm || "").toUpperCase() === "PURGE";
  if (!ok)
    return res
      .status(400)
      .json({ error: 'Confirmation required. Set body.confirm = "PURGE".' });
  ...
  try {
    ...
    // Collections to preserve (user-related)
    ...
    // Drop all collections except user collections
    for (const name of collectionNames) {
      if (!preserveCollections.includes(name)) {
        await mongoose.connection.db!.collection(name).drop();
      }
    }

    // Recreate default users for all portals
    try {
      await ensureDefaultPortalLogins();
    } catch (error) {
      console.error("Failed to recreate default users:", error);
    }

    res.json({
      ok: true,
      message: "All data deleted except user accounts",
...
export async function restoreAll(req: Request, res: Response) {
  ...
  const ok =
    String((req.body as any)?.confirm || "").toUpperCase() === "RESTORE";
  if (!ok)
    return res
      .status(400)
      .json({ error: 'Confirmation required. Set body.confirm = "RESTORE".' });
...
async function applyRetention(dir: string) {
  ...
  const withStat = await Promise.all(
    items.map(async (f) => ({ f, stat: await fs.stat(path.join(dir, f)) })),
  );
  withStat.sort((a, b) => b.stat.mtimeMs - a.stat.mtimeMs);
  const toDelete = withStat.slice(max);
  for (const d of toDelete) {
    try {
      await fs.unlink(path.join(dir, d.f));
    } catch {}
  }
}
