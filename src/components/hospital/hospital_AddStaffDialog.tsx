...
export default function Hospital_AddStaffDialog({ open, onClose, onSave, initial = null, title, submitLabel }: Props) {
  ...
  const [selectedRole, setSelectedRole] = useState<string>(initial?.position || '')
  const [taskUsername, setTaskUsername] = useState<string>('')
  const [taskPassword, setTaskPassword] = useState<string>('')

  useEffect(() => {
    let mounted = true
    ;(async () => {
      try {
        const [shiftRes, rolesRes] = await Promise.allSettled([
          hospitalApi.listShifts() as any,
          (hospitalApi as any).listSidebarRoles?.() as any,
        ])
        if (!mounted) return
        if (shiftRes.status === 'fulfilled') {
          const res = shiftRes.value as any
        ...
        if (rolesRes.status === 'fulfilled') {
          ...
          if (Array.isArray(list) && list.length) {
            const merged = Array.from(new Set([...(list || []), ...fallbackRoles]))
            setRoles(merged)
          } else {
  ...
  useEffect(() => {
    ...
    setSelectedRole(initial?.position || '')
    try {
      const raw = localStorage.getItem('task.users')
      const users = raw ? JSON.parse(raw) : []
      const tu = Array.isArray(users) ? users.find((u: any) => String(u?.id || '') === `h_${String(initial?.id || '')}`) : null
      setTaskUsername(String(tu?.username || ''))
      setTaskPassword('')
    } catch {
      setTaskUsername('')
      setTaskPassword('')
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const staff: PharmacyStaff = {
      ...
      taskUsername: String(fd.get('taskUsername') || ''),
      taskPassword: String(fd.get('taskPassword') || ''),
    }
    onSave(staff)
    onClose()
  }
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <select name="position" value={selectedRole} onChange={e=>setSelectedRole(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
            ...
            <div>
              <label className="mb-1 block text-sm text-slate-700">Task Username</label>
              <input name="taskUsername" value={taskUsername} onChange={e=>setTaskUsername(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder="e.g. staff01" />
            </div>
            <div>
              <label className="mb-1 block text-sm text-slate-700">Task Password</label>
              <input name="taskPassword" value={taskPassword} onChange={e=>setTaskPassword(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" placeholder={initial ? 'Leave blank to keep existing' : 'Set password'} />
            </div>
            <div>
