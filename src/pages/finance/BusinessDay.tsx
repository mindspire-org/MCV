...
export default function Finance_BusinessDay(){
  ...
  async function load(){
    try {
      const res: any = await (financeApi as any).dayStatus()
  ...
  async function doOpen(){
    setLoading(true)
    try { await (financeApi as any).openDay(undefined, note||undefined); setNote(''); await load() } finally { setLoading(false) }
  }
  async function doClose(){
    setLoading(true)
    try { await (financeApi as any).closeDay(undefined, note||undefined); setNote(''); await load() } finally { setLoading(false) }
  }
