...
export default function Pharmacy_StaffManagement(){
  ...
  useEffect(()=>{
    ...
    ;(async () => {
      try {
        ...
        console.error(e)
        if (!mounted) return
        const msg = String(e?.message || e || 'Failed to load staff')
        setNotice({ text: msg || 'Failed to load staff', kind: 'error' })
      }
    })()
  ...
  const upsertTaskPortalUser = (opts: { staffId: string; fullName: string; roleRaw?: any; active: boolean; username?: string; password?: string }) => {
    ...
    try {
      ...
      const users: any[] = Array.isArray(arr) ? arr : []
      const idx = users.findIndex(u => String(u?.id || '') === id)
      const prev = idx >= 0 ? users[idx] : null

      const username = String(opts.username || '').trim() || String(prev?.username || '').trim() || `staff_${staffId.slice(-6)}`
      const passwordInput = String(opts.password || '')
      const password = passwordInput ? passwordInput : String(prev?.password || '').trim() || '123'
      ...
      const next = {
        ...
        username,
        password,
        fullName: String(opts.fullName || '').trim() || username,
        roleId: mapHospitalRoleToTaskRoleId(opts.roleRaw),
        isAdmin: false,
        active: !!opts.active,
        hospitalStaffId: staffId,
      }

      const out = idx >= 0 ? users.map((u, i) => (i === idx ? next : u)) : [next, ...users]
      localStorage.setItem('task.users', JSON.stringify(out))
    } catch {}
  }

  const addStaff = async (s: PharmacyStaff) => {
    const created: any = await hospitalApi.createStaff({
      name: s.name,
    ...
    const staffId = String(created?.staff?._id || created?._id || created?.id || s.id || '').trim()
    upsertTaskPortalUser({
      staffId,
      fullName: s.name,
      roleRaw: s.position,
      active: s.status !== 'Inactive',
      username: (s as any).taskUsername,
      password: (s as any).taskPassword,
    })
    setReloadTick(t=>t+1)
  }
  ...
  const saveEdit = async (updated: PharmacyStaff) => {
    ...
    upsertTaskPortalUser({
      ...
      roleRaw: updated.position,
      active: updated.status !== 'Inactive',
      username: (updated as any).taskUsername,
      password: (updated as any).taskPassword,
    })
    setReloadTick(t=>t+1)
  }
