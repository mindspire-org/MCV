...
export default function Hospital_StoreManagement(){
  ...
  async function submitReceive(){
    const draftLines = receiveForm.lines
    ...
    if (!receiveForm.locationId || !receiveForm.date || !draftLines.length) return

    const badQtyIdx = draftLines.findIndex(l => !(l.qty > 0))
    if (badQtyIdx >= 0){
      alert(`Receive: Line ${badQtyIdx + 1} qty must be > 0.`)
      return
    }
    const badCostIdx = draftLines.findIndex(l => Number.isNaN(l.unitCost) || l.unitCost < 0)
    if (badCostIdx >= 0){
      alert(`Receive: Line ${badCostIdx + 1} unit cost must be 0 or more.`)
      return
    }
    const badLotIdx = draftLines.findIndex(l => !l.lotNo)
    if (badLotIdx >= 0){
      alert(`Receive: Line ${badLotIdx + 1} Lot No is required.`)
      return
    }
    const badExpIdx = draftLines.findIndex(l => !l.expiryDate)
    if (badExpIdx >= 0){
      alert(`Receive: Line ${badExpIdx + 1} Expiry Date is required.`)
      return
    }
  ...
  async function submitIssue(){
    ...
    const lines = issueForm.lines
      .filter(l => String(l.itemId || '').trim() && String(l.qty || '').trim())
      .map(l => ({ itemId: String(l.itemId).trim(), qty: Number(l.qty) }))

    if (!issueForm.date){
      alert('Issue: Date is required.')
      return
    }
    if (!issueForm.locationId){
      alert('Issue: Location is required.')
      return
    }
    if (!issueForm.departmentId){
      alert('Issue: Department is required.')
      return
    }
    if (!lines.length){
      alert('Issue: Add at least one line with item and qty.')
      return
    }

    const badQtyIdx = lines.findIndex(l => !(l.qty > 0) || Number.isNaN(l.qty))
    if (badQtyIdx >= 0){
      alert(`Issue: Line ${badQtyIdx + 1} qty must be > 0.`)
      return
    }
    ...
    try {
      ...
      alert(e?.message || 'Failed to issue stock')
    } finally {
      setIssueBusy(false)
    }
  }
