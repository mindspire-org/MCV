...
export default function Hospital_Backup() {
  ...
  const [settings, setSettings] = useState<AutoSettings>(() => {
    try {
      ...
      return raw
        ? JSON.parse(raw)
        : { enabled: false, minutes: 60, folderPath: "", adminKey: "" };
    } catch {
  ...
  useEffect(() => {
    ...
    const id = setInterval(
      () => doBackup(true),
      Math.max(1, settings.minutes) * 60 * 1000,
    );
  ...
  const deleteAll = async () => {
    ...
    try {
      ...
      setLastBackup(null);
      showBanner("All data cleared (user accounts preserved)");
      logAudit("user_delete", "purge all data (users preserved)");
      setTimeout(() => {
        try {
    ...
    <div>
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-600">
          Manage your application data. It's recommended to create backups
          regularly.
        </p>
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings((s) => ({ ...s, folderPath: e.target.value }))
                }
                placeholder="e.g. C:\\HospitalBackups"
                className="w-full rounded-md border border-slate-300 px-3 py-2 outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200"
              />
              <p className="mt-1 text-xs text-slate-500">
                Note: Browsers download to your default folder; this is for
                reference only.
              </p>
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-slate-700">
                Admin Key (for server admin endpoints)
              </label>
              <input
                value={settings.adminKey || ""}
                onChange={(e) =>
                  setSettings((s) => ({ ...s, adminKey: e.target.value }))
                }
                placeholder="Set to match backend ADMIN_KEY"
                className="w-full rounded-md border border-slate-300 px-3 py-2 outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200"
              />
              <p className="mt-1 text-xs text-slate-500">
                Stored locally and sent as header x-admin-key.
              </p>
            </div>
          </div>
          ...
          <div className="mt-4">
            ...
            {banner && (
              <span className="ml-3 text-sm text-emerald-600">{banner}</span>
            )}
          </div>
        </form>
      </div>
